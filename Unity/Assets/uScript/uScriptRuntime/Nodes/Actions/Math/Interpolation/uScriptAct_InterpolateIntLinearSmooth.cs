...
[FriendlyName("Interpolate Int Linear (Smooth)", "Linearly interpolate an int over time.")]
public class uScriptAct_InterpolateIntLinearSmooth : uScriptLogic
{
   private int m_Start;
   private int m_End;
   ...
   public void Begin(int startValue, int endValue, float time, uScript_Lerper.LoopType loopType, float loopDelay, bool smooth, int loopCount, out int currentValue)
   {
   ...
   // Parameter Attributes are applied below in Resume()
   public void Stop(int startValue, int endValue, float time, uScript_Lerper.LoopType loopType, float loopDelay, bool smooth, int loopCount, out int currentValue)
   {
      m_Lerper.Stop( );
