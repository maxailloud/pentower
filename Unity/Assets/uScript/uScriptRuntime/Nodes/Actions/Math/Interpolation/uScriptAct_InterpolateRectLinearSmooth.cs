...
[FriendlyName("Interpolate Rect Linear (Smooth)", "Linearly interpolate a Rect over time.")]
public class uScriptAct_InterpolateRectLinearSmooth : uScriptLogic
{ 
   ...
   public void Begin(Rect startValue, Rect endValue, float time, uScript_Lerper.LoopType loopType, float loopDelay, bool smooth, int loopCount, out Rect currentValue)
   {
   ...
   // Parameter Attributes are applied below in Resume()
   public void Stop(Rect startValue, Rect endValue, float time, uScript_Lerper.LoopType loopType, float loopDelay, bool smooth, int loopCount, out Rect currentValue)
   {
      m_Lerper.Stop( );
